...
from 5:30pm to 8:40pm PST, collecting chat messages and viewer numbers for the entire duration of the live stream.
...
> - Meaning **{(stats.engagement.engagement).toFixed(2)}% of viewers** engaged by sending at least one message on stream,
...
<BarChart options={topChatterChartOptions} data={topChatterChartData} />
...
<LineChart data={viewershipChartData} />

I just want to point out that viewership peaked when Alex Johns was being choked out. That's all for this analysis.

### engagement

As mentioned in the summary of results, **{stats.engagement.engagement.toFixed(2)}%** of viewers engaged by sending at least one message on stream, I wanted to know how this number compares to other streamers, so I ran this same program on a typical hasanabi stream, on qtcinderella's rain forest charity gala stream, and a late night willneff stream, here are the results:

<BarChart
  options={viewerEngagementChartOptions}
  data={viewerEngagementChartData}
/>

Engagement is calculated by diving the amount of unique chatters by the average viewership, and as you can see Wubby has the most out of the 4 streamers, though I think these result aren't that concrete because the engagement here is calculated from only one live stream of each streamer, and the engagement of a streamer can vary wildly depending on the content of the stream, the time of day, the day of the week, etc. So don't conclude anything significant from these results.
...
<Table
  headers={["streamer", "% of messages by 20% of chatters"]}
  data={[
    ["paymoneywubby", "76.23%"],
    ["willneff", "73.44%"],
    ["qtcinderella", "75.12%"],
...
Holy crap, looks like is pretty consistent across all of our limited 4 measurements, I guest getting _oneguyed_ is just inevitable if you're a streamer.
